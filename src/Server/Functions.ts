...
export const isNameInUse = ({ name, users }: { name: string; users: Users }) => {
...
export const resetPlayerState = ({ user, users }: { user: User; users: Users }) => {
...
export const createUserWithLocation = ({ name, socket }: { name: string; socket: Socket }) =>
  new Promise<User>((resolve, reject) => {
    ...
    iplocation(ip, [])
      .then((res) => {
        const user = createUser(name, socket.id, ip, res.countryCode, res.latitude, res.longitude);
        resolve(user);
      })
      .catch((err) => {
        reject(err);
